...
class NutNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.srv_proj = nn.Linear(emb_dim * 4 + 6, d_model)

        self.usr_f_attn = Encoder(d_model, d_model, head, 2, 0.2)
        ...
        self.qos_net = PreLayer(emb_dim * 8 + 19 + d_model, [64, 32, 16, 8, p])
    ...
    def forward(self, tra, u_inv, srv, e_inv, mask, info, qos, edge_index):
        ...
        usr_emb = usr_emb.unsqueeze(0).unsqueeze(2).expand(t, -1, k, -1)  # [t, n, k, emb_dim]
        u_inv = u_inv @ svc_emb  # [t, n, k, emb_dim * 4]
        usr_mat = torch.concat((usr_emb, tra, u_inv), -1)  # [t, n, k, emb_dim * 5 + 4]
        usr_mat = self.usr_proj(usr_mat)  # [t, n, k, d_model]
        usr_mat = usr_mat + positional_encoding(usr_mat, usr_mat.device).unsqueeze(0).unsqueeze(1).expand(t, n, -1, -1)
        u_mask = generate_causal_mask_pytorch(k).unsqueeze(0).unsqueeze(1).expand(t, n, -1, -1).to(usr_mat.device)
        usr_mat = self.usr_f_attn(usr_mat, u_mask)
        usr_mat = usr_mat.transpose(-2, -3)

        e_inv = e_inv @ svc_emb  # [t, m, k, emb_dim * 4]
        srv_mat = self.srv_attr[:, -3:].unsqueeze(0).unsqueeze(2).expand(t, -1, k, -1)  # [t, m, k, 3]
        srv_mat = torch.concat((srv_mat, e_inv), -1)  # [t, m, k, emb_dim * 4 + 3]

        srv_mat = torch.concat((srv_mat, srv), -1)  # [t, m, k, emb_dim * 4 + 6]
        srv_mat = self.srv_proj(srv_mat)  # [t, m, k, d_model]
        srv_mat = srv_mat.transpose(-2, -3)

        tem_srv = self.decoder(usr_mat, srv_mat, mask)  # [t, k, m, d_model]

        tem_srv = tem_srv[:, -1, :, :]  # [t, m, d_model]

        usr_mat = usr_mat[:, -1, :, :]  # [t, n, d_model]

        for net in self.gcn:
            tem_srv = net(tem_srv, edge_index)  # [t, m, 16]

        usr_mat = usr_mat[inverse_indices, info[:, 0]]  # [t, d_model]
        tem_srv = tem_srv[inverse_indices, info[:, 1]]  # [t, 16]
        srv_emb = srv_emb[info[:, 1]]  # [t, emb_dim * 4]
        svc_emb = svc_emb[info[:, 2]]  # [t, emb_dim * 4]
        x = torch.concat((usr_mat, tem_srv, srv_emb, svc_emb), -1)  # [t, emb_dim * 8 + 19 + d_model]
