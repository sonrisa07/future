...
class MyDataset(Dataset):
    ...
    def __init__(self,
        ...
        for st in range(0, len(grouped) - k):
            ...
            window = np.stack(
                [window[window['timestamp'] == t][['computing_load', 'storage_load', 'bandwidth_load']].to_numpy() for t
                 in range(st, st + k)])
            load.append(window)
        ...
        t_df = t_df.groupby(['timestamp', 'eid'], as_index=False).agg({
...
class NutNet(nn.Module):
    ...
    def __init__(self, k, srv_res, svc_res, tra_dim=8, emb_dim=8, emb_load_dim=16):
        ...
        self.load_net = nn.ModuleList([
            STConv(srv_res.shape[0], emb_load_dim // 2 + 3 * emb_dim,
                   64,
                   32,
                   3,
                   3),
            STConv(srv_res.shape[0], 32,
                   64,
                   56,
                   3,
                   3)
    ...
    def forward(self, tra, info, load, svc, edge_index):
        ...
        # ****** 服务器资源embedding ******
        e_rate = srv_res_emb.unsqueeze(0).expand(self.k, -1, -1)  # k * N_srv * (3*emb_dim)
        # *******************************

        # **** 服务器资源 + 服务器负载 ****
        e_load = self.e_load(torch.concat((load, e_rate.unsqueeze(0).expand(load.shape[0], -1, -1, -1)),
                                          -1))  # t * k * N_srv * (3 + 3 * emb_dim) -> t * k * N_srv * emb_load_dim // 2
        ...
        # ********服务器时空预测**********
        e_load = torch.concat((e_load, svc_tot), -1)  # t * k * N_srv * (emb_load_dim // 2 + 3 * emb_dim)
